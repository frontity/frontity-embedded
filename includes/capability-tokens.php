 ...
 */
class Frontity_Embedded_Capability_Tokens {
  ...
  public static function setup() {
    $payload = Frontity_Embedded_Capability_Tokens::get_payload_from_token();
    Frontity_Embedded_Capability_Tokens::$payload = $payload;

    // Filter that modifies user capabilities during runtime.
    if ( defined( 'REST_REQUEST' ) && REST_REQUEST && $payload ) {
      add_filter( 'user_has_cap', 'Frontity_Embedded_Capability_Tokens::user_has_cap', 20, 4);
    }
  }
  ...
  public static function generate( $payload ) {
    ...
    $payload = array(
      "iat" => $issued_at,
      // Expire the token in 60 seconds.
    ...
    return JWT::encode($payload, Frontity_Embedded_Capability_Tokens::get_private_key());
  }
  ...
  public static function check_capability( $args ) {
    $payload = (array) Frontity_Embedded_Capability_Tokens::$payload;

    // Do not authenticate if the token was not generated by Frontity.
    if ( $payload['generator'] !== 'frontity' ) return false;
  ...
  public static function user_has_cap( $allcaps, $caps, $args, $user ) {
    ...
    if ( Frontity_Embedded_Capability_Tokens::check_capability( $args )) {
      foreach ( $caps as $cap ) {
        $allcaps[ $cap ] = true;
      }
    }
  ...
  private static function get_payload_from_token() {
    ...
    try {
      ...
      $payload = JWT::decode(
        $token,
        Frontity_Embedded_Capability_Tokens::get_private_key(),
        array('HS256')
      );
    } catch (Exception $e) {
      // Token is not valid.
